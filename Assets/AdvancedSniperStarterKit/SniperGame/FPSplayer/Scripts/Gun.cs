...
public class Gun : MonoBehaviour
{
	...
	public virtual void Reload ()
	{
		...
		if (gunState == 0) {
			...
			Clip = 0;
			gunState = 3;
			GameManager.Instance.SetBullets(0, true);
		}
	}
	...
	public virtual void  Shoot ()
	{
		...
		if (timefire + FireRate < Time.time) {
			if (gunState == 0) {
				if (AmmoIn > 0) {
					GameManager.Instance.SetBullets(-1, false);
					...
					for (int i = 0; i < BulletNum; i++) {
						if (Bullets) {
							...
							if (MuzzleFX != null && MuzzlePoint != null) {
								muzzleCopy= (GameObject)GameObject.Instantiate (MuzzleFX, MuzzlePoint.transform.position, MuzzlePoint.transform.rotation);
								muzzleCopy.transform.SetParent (MuzzlePoint.transform);
								GameObject.Destroy (muzzleCopy.gameObject, 3);
							}
						}	
					}
					boltout = false;
