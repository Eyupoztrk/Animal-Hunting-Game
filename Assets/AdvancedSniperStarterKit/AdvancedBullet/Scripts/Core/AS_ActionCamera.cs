...
public class AS_ActionCamera : MonoBehaviour
{
	...
	//private static float initialMaximumTimeScale = 1;
	...
	[HideInInspector]
	public GameObject TargetShouldHit;
	[HideInInspector]
	public Vector3 PositionFirstDetect;
	[HideInInspector]
	...
	void Start ()
	{
		...
		//initialMaximumTimeScale = Time.maximumDeltaTime;
	...
	void cameraUpdate ()
	{
		...
		if (onWallCollision) {
			...
			this.transform.position = Vector3.Lerp (this.transform.position, cameraPosition + ((-this.transform.forward) * BaseDistance), Time.fixedUnscaledDeltaTime * 30);
		}
			
		if (MainCamera) {
			MainCamera.fieldOfView = Mathf.Lerp (MainCamera.fieldOfView, FOVTarget, Time.fixedUnscaledDeltaTime * 30 * FOVspeed);	
		}
	...
	void cameraCollision ()
	{
		...
		if (InAction) {
			...
			// for debuging if you want to see a line of camera detection
			// need Line renderer component on this object
			/*LineRenderer line = this.GetComponent<LineRenderer> ();
			line.SetPosition (0, lookatpoint);
			...
			// This is for detect ground
			/*if (Physics.Raycast (this.transform.position, -Vector3.up, out hit, ColliderOffset)) {
				if (hit.collider.gameObject != this.gameObject && hit.collider.gameObject.layer != IgnoreCameraLayer) {
					cameraPosition.y = hit.point.y + ColliderOffset;
				}
			}*/
	...
	void LateUpdate ()
	{
		...
		// to check if a bullet miss a targetl
